 ...
 *  and a config.txt specification. The config.txt specification is
...
class Configurer {
   ...
   private Tokenizer scan;         // get tokens from config.txt
   ...
   Configurer( DeviceFactory theFactory ) {
      ...
      factory = theFactory;
         ...
         // hook up the stuff that does not depend on config.txt
      ...
      /**
       *  Process a config.txt specification and use it in conjunction
       ...
       *  operation of a recursive descent parser to parse the config.txt
       *  specification.
       *
       *     @pre config.txt is not null
       *    @post parses the config.txt file and uses it to configure
   ...
   void configure( Reader configuration ) {
      ...
      if ( configuration == null )
         throw new IllegalArgumentException( "No config.txt file reader." );
      ...
      tokenType = scan.nextToken();
         ...
         // if config.txt fails, tell the ui controller
      try {
         configFile();
      }
      catch ( StartupException e ) {
   ...
   Irrigator getIrrigator() { return irrigator; }
      ...
      /**
       *  Parse the config.txt file specification. This operation
   ...
   private void valveSpec( String zoneID ) throws StartupException {
       ...
       *  The error message states that there is a config.txt parse error
